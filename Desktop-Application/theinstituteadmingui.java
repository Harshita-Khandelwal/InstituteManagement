...
class TheInstituteAdminGUI extends Common
{
	private JPanel pleftce;
	private JButton bstuprofile,bquery,bcourse,buser,bfee_details,bfaculty,blogout;
	TheInstituteAdminGUI()
	{
		bstuprofile=new JButton("Student");
		bquery=new JButton("Query");
		bcourse=new JButton(" Course/Batch ");
		...
		bcourse.addActionListener((e)->{new Course(framepanel,desktop,jf,1);});
		buser.addActionListener((e)->{new User(framepanel,desktop,jf);});
		bfee_details.addActionListener((e)->{new Fee(framepanel,desktop,jf,1);});
		bfaculty.addActionListener((e)->{new Faculty(framepanel,desktop,jf,1);});
